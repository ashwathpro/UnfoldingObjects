...
bool ExtendedMesh::RebuildMesh(qreal scale) {
    if (m_triangleMesh == NULL || scale> MAX_VALUE/100.0 || scale <MIN_VALUE/100.0) return false;
    else if (m_scale == scale) return false;
    for (int i=0; i<m_faces.size(); ++i) {
        m_faces[i]->BuildSmallerFace(scale);
        QVector <ExtendedEdge* > edges = m_faces[i]->GetEdges();       
        for (int j=0; j<edges.size(); ++j) {
            ExtendedFace* f = edges[j]->GetAnotherFace(m_faces[i]);
            edges[j]->BuildConnector(m_faces[i], f, scale);
        }
    }
    m_scale = scale;
    BuildFoldedMesh();
...
bool ExtendedMesh::BuildFoldedMesh() {
    if (isInitialized == false) InitialExtendedMesh();
    ...
    // create folded faces
    for (int i=0; i<m_faces.size(); ++i) {
        QVector <QVector3D> vts = m_faces[i]->GetFoldedSmallerFace();
    ...
    for (int i=0; i<m_edges.size();++i) {
        QVector <ExtendedFace*> faces = m_edges[i]->IncidentFaces();
        assert(faces.size() == 2);
        ExtendedFace* parent = NULL;

        if (faces[0]->IsChildFace(faces[1]) == true) parent = faces[0];
        else if (faces[1]->IsChildFace(faces[0]) == true) parent = faces[1];
        else continue;

        QVector <QVector <QVector3D> > curves = m_edges[i]->GetFoldedConnector(parent);
...
bool ExtendedMesh::Unfolding() {
    if (isInitialized == false) InitialExtendedMesh();
    if (MSTQueue.empty()) return false;
...
QVector3D ExtendedMesh::GetFirstUnfoldedPos(const QVector3D &oldPos, ExtendedFace *face) {
    ...
    for (int i=0; i<foldedSmallFace.size(); ++i) {
        if (foldedSmallFace[i] == oldPos) id = i;
    }
...
bool ExtendedMesh::UnfoldFace(ExtendedFace *last, ExtendedFace *next, ExtendedEdge *edge, QVector3D normal) {
    ...
    if (vertical.dotProduct(vertical, C)>0)
        vertical = -vertical;
    vertical.normalize();
    QVector3D s1 = next->GetFoldedSmallerFace()[2] - next->GetFoldedSmallerFace()[0];
    QVector3D s2 = next->GetFoldedSmallerFace()[1] - next->GetFoldedSmallerFace()[0];
...
bool ExtendedMesh::AddFace(QVector <Vertex*>& vertices) {
    if (vertices.size()<2) return false;
    QVector <ExtendedEdge* > currentEdges;
    for (int i=0; i<vertices.size(); ++i) {
        int firstIndex = i;
        int lastIndex = (i == vertices.size()-1) ? 0 : i+1;
        ExtendedEdge* current = EdgeExisting(vertices[firstIndex], vertices[lastIndex]);
        if (!current) {
            try {
                ExtendedEdge* edge = new ExtendedEdge(vertices[firstIndex], vertices[lastIndex]);
...
bool ExtendedMesh::InitialExtendedMesh() {
    if (m_faces.size()<1) return false;
...
bool ExtendedMesh::MST(ExtendedFace *first) {
    ...
    MSTQueue.clear();
    QQueue <ExtendedFace*> queue;
    QSet <ExtendedFace*> faceUsed;
    queue.push_back(first);
    faceUsed.insert(first);

    while (!queue.empty()) {
        ExtendedFace* current = queue.first();
        queue.pop_front();
        MSTQueue.push_back(current);
        QVector <ExtendedEdge*> edges = current->GetEdges();
        QVector <ExtendedFace*> incidentFaces(edges.size(), NULL);
        ...
        for (int i=0; i<incidentFaces.size(); ++i) {
            if (faceUsed.find(incidentFaces[i]) == faceUsed.end()) {
                current->AddTreeChildFace(incidentFaces[i]);
                queue.push_back(incidentFaces[i]);
                edges[i]->SetAsConnector();
                faceUsed.insert(incidentFaces[i]);
            } else
                current->AddTreeNotChildFace(incidentFaces[i]);
        }
    }
    return true;
}
